...
frappe.views.ListView = class ListView extends frappe.views.BaseList {
	...
	set_primary_action() {
		if (this.can_create && !frappe.boot.read_only) {
			const doctype_name = _(frappe.router.doctype_layout) || _(this.doctype);
	...
	get_no_result_message() {
		...
		let no_result_message =
			filters && filters.length
				? _("No {0} found", [_(this.doctype)])
				: _("You haven't created a {0} yet", [_(this.doctype)]);
	...
	get_like_html(doc) {
		...
		<span class="likes-count">
			${liked_by.length > 99 ? _("99") + "+" : _(liked_by.length || "")}
		</span>`;
	}
	...
	toggle_tags() {
		...
		const preview_label = this.tags_shown ? _("Hide Tags") : _("Show Tags");
